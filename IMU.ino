...
static int32_t accLPF32[3]    = {0, 0, ACC_1G<<ACC_LPF_FACTOR};
...
void getEstimatedAttitude(){
  ...
  att.angle[PITCH] = _atan2(EstG32.V.Y , InvSqrt(sqGX_sqGZ)*sqGX_sqGZ);
    ...
    cosZ = EstG.V.Z * 100.0f / ACC_1G;                                                        // cos(angleZ) * 100 
...
uint8_t getEstimatedAltitude(){
  int32_t  BaroAlt;
  static float baroGroundTemperatureScale, baroGroundPressureSum, vel = 0.0f;
  static uint16_t previousT;
  uint16_t currentT = micros();
  uint16_t dTime;
  ...
  //BaroAlt = ( logBaroGroundPressureSum - log(baroPressureSum) ) * baroGroundTemperatureScale; // in cemtimeter     BAD !!! but why ?
  BaroAlt = log( baroGroundPressureSum / baroPressureSum ) * baroGroundTemperatureScale; // in cemtimeter 
...
debug[1] = sonarAlt;
    if(sonarAlt > 0 && f.SMALL_ANGLES_25) {      // use sonar only if valid sonar data, and with inclinations below 25 degres
      ...
      //if(BaroAlt > sonarAlt) baroGroundPressureSum--;       // adjust baro ground altitude according to sonar data
      //if(BaroAlt < sonarAlt) baroGroundPressureSum++;
...
debug[2] = alt.EstAlt;
    ...
    if(f.FAILSAFE_ON)
        BaroPID -= constrain(conf.pid[PIDALT].D8 * alt.vario >>7, -80, 80);
    else    
