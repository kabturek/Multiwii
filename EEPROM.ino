...
void update_constants() {
    ...
    conf.vbatlevel_warn = VBATLEVEL_WARN;
  ...
  #if defined (FAILSAFE)
    conf.rth_altitude = (int32_t)(RETURN_ALTITUDE*100L);
  #endif
  #ifdef POWERMETER
  ...
  #endif
  #if defined(SERVO) && defined(SERVO_MIN)
  ...
  writeParams(0);
    ...
    global_conf.RF_Header[0] = (BIND_SIGNATURE >> 24) & 0xFF;
    global_conf.RF_Header[1] = (BIND_SIGNATURE >> 16) & 0xFF;
    global_conf.RF_Header[2] = (BIND_SIGNATURE >> 8 ) & 0xFF;
    global_conf.RF_Header[3] = (BIND_SIGNATURE >> 0 ) & 0xFF;
    writeGlobalSet(0);
  #endif
}
...
void LoadDefaults() {
  ...
  #ifdef FIXEDWING
    conf.pid[PIDALT].P8   = 10; conf.pid[PIDALT].I8   = 0; conf.pid[PIDALT].D8   = 20;
  #else
  ...
  #ifdef FIXEDWING
    conf.pid[PIDNAVR].P8 = 30;  conf.pid[PIDNAVR].I8 = 0;   conf.pid[PIDNAVR].D8 =20;
  #else
