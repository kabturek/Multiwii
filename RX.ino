...
volatile uint16_t rcValue[RC_CHANS];
  ...
  static uint8_t rcChannel[RC_CHANS] = {PITCH,YAW,THROTTLE,ROLL,AUX1,AUX2,AUX3,AUX4,AUX5,AUX6,AUX7,AUX8,12,13,14,15,16,17};
  static uint16_t sbusIndex=0;
#elif defined(SPEKTRUM)
  static uint8_t rcChannel[RC_CHANS] = {PITCH,YAW,THROTTLE,ROLL,AUX1,AUX2,AUX3,AUX4,AUX5,AUX6,AUX7,AUX8};
...
void configureReceiver() {
  for(uint8_t i=0; i<RC_CHANS; i++) {  // Init RC channels value
    rcValue[i] = MIDRC;
    rcData[i]  = MIDRC;
  }
...
/**************************************************************************************/
  ...
  ISR(RX_PC_INTERRUPT) { //this ISR is common to every receiver channel, it is call everytime a change state occurs on a RX input pin
    ...
    #if defined(FAILSAFE) && !defined(PROMICRO)
      if(!rcOptions[BOXFAILSAFE] && GoodPulses==(1<<THROTTLEPIN)+(1<<YAWPIN)+(1<<ROLLPIN)+(1<<PITCHPIN)) { // If all main four chanells have good pulses, clear FailSafe counter
        GoodPulses = 0;
        failsafeCnt >>= 1;
      }
    #endif
    ...
    ISR(INT6_vect){ 
      ...
      if(!(PINE & (1<<6))){
        ...
        if(900<diff && diff<2200){
          rcValue[3] = diff;
          #if defined(FAILSAFE)
           if(!rcOptions[BOXFAILSAFE] && diff>FAILSAFE_DETECT_TRESHOLD) {        // if Throttle value is higher than FAILSAFE_DETECT_TRESHOLD)
             failsafeCnt >>= 1;   // If pulse present on THROTTLE pin (independent from ardu version), clear FailSafe counter  - added by MIS
           }
          #endif 
        }
      }else last = now; 
    ...
    if(diff>3000) chan = 0;
      ...
      if(900<diff && diff<2200 && chan<RC_CHANS ) {   //Only if the signal is between these values it is valid, otherwise the failsafe counter should move up
        rcValue[chan] = diff;
          ...
          if(!rcOptions[BOXFAILSAFE] && GoodPulses==0x0F) {
            GoodPulses = 0;
            failsafeCnt >>= 1;
          }
        #endif
...
/**************************************************************************************/
 ...
 void  readSBus(){
  while(SerialAvailable(1)){
    ...
    if(sbusIndex==25){
      sbusIndex=0;
      rcValue[0]  = ((sbus[1]|sbus[2]<< 8) & 0x07FF)/2+SBUS_MID_OFFSET; // Perhaps you may change the term "/2+976" -> center will be 1486
      ...
      #if defined(FAILSAFE)
      if(!rcOptions[BOXFAILSAFE] && !((sbus[23] >> 3) & 0x0001))
        failsafeCnt >>= 1;   // clear FailSafe counter
      #endif
    }
  }        
}
...
void readSpektrum() {
  ...
  if (spekFrameFlags == 0x01) {   //The interrupt handler saw at least one valid frame start since we were last here. 
    if (SerialAvailable(SPEK_SERIAL_PORT) == SPEK_FRAME_SIZE) {  //A complete frame? If not, we'll catch it next time we are called. 
      ...
      spekFrameFlags = 0x00;
      #if defined(FAILSAFE)
        if(!rcOptions[BOXFAILSAFE]) {
          failsafeCnt >>= 1;   // Valid frame, clear FailSafe counter
        }
      #endif
