...
/*** I2C address ***/
  ...
  #define MS561101BA_ADDRESS2 0x76 //CBR=1 0xEC I2C address when pin CSB is connected to HIGH (VCC)
#endif
...
// Functions adapted for dual-baro option on kuki_07M board. 
// With standard single baro, no differences in generated code and code size - comiler optimize it.
...
static struct {
  ...
  uint8_t  state;
  uint32_t deadline;
} ms561101ba_ctx[BARO_NUM];

void i2c_MS561101BA_reset(){
  i2c_writeReg(MS561101BA_ADDRESS, MS561101BA_RESET, 0);
  #if defined(MS561101BA_DUAL)
    i2c_writeReg(MS561101BA_ADDRESS2, MS561101BA_RESET, 0);
  #endif  
}

void i2c_MS561101BA_readCalibration(){
  union {uint16_t val; uint8_t raw[2]; } data;
  for(uint8_t nb=0; nb<BARO_NUM; nb++) {
    for(uint8_t i=0;i<6;i++) {
      i2c_rep_start((MS561101BA_ADDRESS-nb)<<1);
      i2c_write(0xA2+2*i);
      delay(10);
      i2c_rep_start(((MS561101BA_ADDRESS-nb)<<1) | 1);//I2C read direction => 1
      delay(10);
      data.raw[1] = i2c_readAck();  // read a 16 bit register
      data.raw[0] = i2c_readNak();
      ms561101ba_ctx[nb].c[i+1] = data.val;
    }
  }
}
...
void  Baro_init() {
  ...
  ms561101ba_ctx[0].deadline = currentTime+10000; 
}

// read uncompensated temperature value: send command first
void i2c_MS561101BA_UT_Start() {
  for(uint8_t nb=0; nb<BARO_NUM; nb++) {
    i2c_rep_start((MS561101BA_ADDRESS-nb)<<1);  // I2C write direction
...
void i2c_MS561101BA_UP_Read () {
  for(uint8_t nb=0; nb<BARO_NUM; nb++) {
    i2c_rep_start((MS561101BA_ADDRESS-nb)<<1);
    i2c_write(0);
    i2c_rep_start(((MS561101BA_ADDRESS-nb)<<1) | 1);
    ms561101ba_ctx[nb].up.raw[2] = i2c_readAck();
    ms561101ba_ctx[nb].up.raw[1] = i2c_readAck();
    ms561101ba_ctx[nb].up.raw[0] = i2c_readNak();
  }
}
...
void i2c_MS561101BA_UT_Read() {
  for(uint8_t nb=0; nb<BARO_NUM; nb++) {
    ...
    i2c_write(0);
    i2c_rep_start(((MS561101BA_ADDRESS-nb)<<1) | 1);
    ms561101ba_ctx[nb].ut.raw[2] = i2c_readAck();
    ms561101ba_ctx[nb].ut.raw[1] = i2c_readAck();
    ms561101ba_ctx[nb].ut.raw[0] = i2c_readNak();
  }
}
...
void i2c_MS561101BA_Calculate() {
  ...
  #if defined(MS561101BA_DUAL)
    int32_t baro_pressure[2];
    #define baro_press  baro_pressure[nb]
  #else
    #define baro_press  baroPressure
  #endif
  for(uint8_t nb=0; nb<BARO_NUM; nb++) {
    int64_t dT       = (int32_t)ms561101ba_ctx[nb].ut.val - ((int32_t)ms561101ba_ctx[nb].c[5] << 8);
    baroTemperature  = 2000 + ((dT * ms561101ba_ctx[nb].c[6])>>23);
    int64_t off      = ((uint32_t)ms561101ba_ctx[nb].c[2] <<16) + ((dT * ms561101ba_ctx[nb].c[4]) >> 7);
    int64_t sens     = ((uint32_t)ms561101ba_ctx[nb].c[1] <<15) + ((dT * ms561101ba_ctx[nb].c[3]) >> 8);
    ...
    baro_press   = (( (ms561101ba_ctx[nb].up.val * sens ) >> 21) - off) >> 15;
  }
  #if defined(MS561101BA_DUAL)
    baroPressure = (baro_pressure[0] + baro_pressure[1]) / 2;
  #endif  
}

//return 0: no data available, no computation ;  1: new value available  ; 2: no new value, but computation time
uint8_t Baro_update() {                            // first UT conversion is started in init procedure
  if (currentTime < ms561101ba_ctx[0].deadline) return 0; 
  ms561101ba_ctx[0].deadline = currentTime+10000;  // UT and UP conversion take 8.5ms so we do next reading after 10ms 
  if (ms561101ba_ctx[0].state == 0) {
    i2c_MS561101BA_UT_Read(); 
    i2c_MS561101BA_UP_Start(); 
    Baro_Common();                              // moved here for less timecycle spike
    ms561101ba_ctx[0].state = 1;
    return 1;
  } else {
    i2c_MS561101BA_UP_Read();
    i2c_MS561101BA_UT_Start(); 
    i2c_MS561101BA_Calculate();
    ms561101ba_ctx[0].state = 0; 
    return 2;
  }
}
#endif
...
void Sonar_update() {
  ...
  if(srf08_ctx.range[0] > 0 && srf08_ctx.range[0] < SONAR_MAX_RANGE)
...
// ************************************************************************************************************
// HC-SR04 or other generic ECHO-PULSE Ultrasonic Sonar
// ************************************************************************************************************

static uint8_t TrigCnt;
void Sonar_init()
{
  pinMode(SONAR_GEP_EchoPin,INPUT);
  pinMode(SONAR_GEP_TriggerPin,OUTPUT);
...
ISR(SONAR_GEP_EchoPin_PCINT_vect) {
  static uint32_t SONAR_GEP_Time = 0;
  static uint32_t sonarfil;  
  if (SONAR_GEP_EchoPin_PIN & (1<<SONAR_GEP_EchoPin_PCINT)) { 
    SONAR_GEP_Time = micros();
  } else {
    SONAR_GEP_Time = micros() - SONAR_GEP_Time;
    TrigCnt = 0;
    SONAR_Present = 1;
    if (SONAR_GEP_Time <= SONAR_MAX_RANGE*SONAR_GENERIC_SCALE) {
      sonarfil -= sonarfil/8;
      sonarfil += SONAR_GEP_Time / SONAR_GENERIC_SCALE;    // sonar LPF
      sonarAlt = sonarfil/8;
    } else { sonarAlt = -1; }    // out of range
  }     
}

void Sonar_update()
{
  static uint32_t sonar_deadtime;
  if (currentTime >= sonar_deadtime)
  {
    sonar_deadtime = currentTime + 60000;    // 60ms trigger interval
    SONAR_GEP_TriggerPin_PIN_HIGH;
    delayMicroseconds(10);
    SONAR_GEP_TriggerPin_PIN_LOW; 
    if(++TrigCnt > 3) sonarAlt = -1;
    if(TrigCnt > 10) SONAR_Present = 0;
  }
}
