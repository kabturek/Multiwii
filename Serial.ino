  ...
  #define INBUF_SIZE 64
#endif
#if defined(GPS_SERIAL) || defined(MEGA)
...
#if defined(SPEK_BIND) || defined(ELERES_RX)
  #define BIND_CAPABLE 1    //Used for Spektrum and eLeReS; can be used in the future for any RX type that needs a bind and has a MultiWii module.
#else
...
const uint32_t capability = 0x80000000 + BIND_CAPABLE + BOARD_OPTIONS;
...
#define MSP_OSD_DISPLAY          155   //out message         Display string on Misio-OSD
...
static uint8_t osd_port;
void serialSendOsdString(uint8_t control, const char *string) {    // sending string via special MSP command to OSD for LCD emulation
  uint8_t i,c;                                                     // only FC -> OSD communication without request from OSD, not bidirectional
  CURRENTPORT = osd_port;                  // last used port for MSP_ATTITUDE command - probably port with connected OSD
  cmdMSP[CURRENTPORT] = MSP_OSD_DISPLAY;   // command
  headSerialReply(22);                     // send MSP command with constant length of 21 bytes
  serialize8(control);                     // control word - attributes and line number
  for (i=0;i<21;i++) {                     // one line has 20 characters long
    c = *string++;
    if(c==0) break;                        // if end of string
    serialize8(c);                         // sending character
  }
  for (;i<21;i++) serialize8(0);           // fill to 20 bytes of data
  tailSerialReply();                       // send checksum
}
#endif
...
void serialCom() {
  ...
  for(n=0;n<UART_NUMBER;n++) {
    ...
    #define SBUS_COND
      ...
      #define SBUS_COND  && (CURRENTPORT != 1)
    #endif
...
void evaluateCommand() {
  ...
  switch(cmdMSP[CURRENTPORT]) {
   ...
   case MSP_SET_MISC:
     ...
     conf.minthrottle = set_misc.b;
     #ifdef FAILSAFE 
       conf.rth_altitude = (uint32_t)set_misc.d * 100L;
       ...
       conf.vbatlevel_warn   = set_misc.k;
   ...
   case MSP_MISC:
     ...
     break;
   #endif  
   #if defined(ENABLE_MOTORTEST)
     case MSP_SET_MOTOR:
       if(read16() == 0x45CB) {                    // magic word for enabling MotorTest feature (0x45CB) - compatible with ServoMiscConf_v07
         motorTestNumber = read8();                // motor number for testing
         motorValueData   = read16();              // motor value
       } else motorTestNumber = 0;
       break;
   #endif
   ...
   case MSP_STATUS:
     ...
     #endif
     #if defined(FAILSAFE)
       if(f.FAILSAFE_ON) tmp |= 1<<BOXFAILSAFE;
     #endif
     if(f.ARMED) tmp |= 1<<BOXARM;

     st.cycleTime        = cycleTime;
     st.i2c_errors_count = i2c_errors_count;
     st.sensor           = ACC|BARO<<1|magInit<<2|GPS_Present<<3|SONAR_Present<<4;
     st.flag             = tmp;
     st.set              = global_conf.currentSet;
     s_struct((uint8_t*)&st,11);
     break;
   case MSP_RAW_IMU:
       // Send the unfiltered Gyro & Acc values to gui while motortest enabled.
     if(motorTestNumber) {
   ...
   case MSP_RAW_GPS:
     ...
     gps_s.a=f.GPS_FIX;
     gps_s.b=GPS_numSat;
     gps_s.c=GPS_coord[LAT];
     gps_s.d=GPS_coord[LON];
     gps_s.e=GPS_altitude;
     gps_s.f=GPS_speed;
     gps_s.g=GPS_ground_course;        // added since r1172
     memcpy(gps_s.h,GPS_time,6);       // new - gps time 
     s_struct((uint8_t*)&gps_s,22);
     break;
   case MSP_COMP_GPS:
     headSerialReply(5);
...
// END FRSKY ADD
   ...
   case MSP_BIND:
     ...
     break;
   #endif
   #if defined(ELERES_RX) && defined(ELERES_BIND)
   case MSP_BIND:
     if(!f.ARMED) eLeReS_Bind();  
     headSerialReply(0);
     break;
   #endif
...
uint8_t SerialAvailable(uint8_t port) {
    ...
    #endif
  #endif
  return ((uint16_t)serialHeadRX[port] + RX_BUFFER_SIZE - serialTailRX[port])%RX_BUFFER_SIZE;
}
