...
// Modified version by MIS
// !!! USE GUI FROM VERSION 2.3 !!!
...
/*************************************************************************************************/
  ...
  /**************************************************************************************/
    ...
    /***************************    Combined IMU Boards    ********************************/
      ...
      //#define KUKI_063           // not tested
      //#define KUKI_07M
      //#define PROFLYER_v31	   // available here: www.proflyer.eu
      ...
      //#define HK_MultiWii_PRO    // MEGA2560 + ITG3205 + BMA180 + BMP085 + HMC5583L
      ...
      //#define MultiWii_PRO_Ez3   // MEGA+MPU6050+MS5611BA+HMC5883L   http://witespyquad.gostorego.com
    ...
    /***************************    independent sensors    ********************************/
      ...
      */
      ...
      #define SONAR_GENERIC_SCALE 58 	// scale for ranging conversion (hc-sr04 is 58)
      #define SONAR_MAX_RANGE 500 		// in cm
...
/*************************************************************************************************/
   ...
   /* optionally disable stick combinations to arm/disarm the motors.
    ...
    //#define FORCE_SERVO_RATES      {30,30,0,100,100,100,100,100} // 0 = normal, 1= reverse or From -120% to 120%
  ...
  /***********************          Cam Stabilisation             ***********************/
    ...
    // trigger interval can be changed via (*GUI*) - (SERVO3 Midpoint) or via AUX channel
    //#define CAMTRIG
  ...
  /***********************          Airplane                       ***********************/
   ...
   //#define FLAPSPEED     2             // Make flaps move slowm Higher value is Higher Speed.
...
/*************************************************************************************************/
  ...
  /**************************************************************************************/
    ...
    /*****************    eLeReS compatibile reciver on RFM22B module   ********************/
      ...
      #define BASE_FREQ          435000          // 435.000MHz
      #define BIND_SIGNATURE     0x72663112      // Some random Bind Signature
      #define CHANNELS_ORDER     ROLL,PITCH,THROTTLE,YAW,AUX1,AUX2,AUX3,AUX4,AUX5,AUX6,AUX7,AUX8
      #define TELEMETRY_ENABLE
      #define TELEMETRY_POWER    7    // 0=1mW, 1=2mW, 2=3mW, 3=6mW, 4=13mW, 5=25mW, 6=50mW, 7=100mW
      //#define TELEMETRY_SEND_DEBUG
      #define LOCALIZER_ENABLE
...
/*************************************************************************************************/
  ...
  /**************************************************************************************/
    ...
    /******                Gyro smoothing    **********************************/
      ...
      //#define GYRO_SMOOTHING {10, 10, 5}    // separate averaging ranges for roll, pitch, yaw

    /************************    Moving Average Gyros    **********************************/
      //#define MMGYRO 10                      // Active Moving Average Function for Gyros
      //#define MMGYROVECTORLENGTH 8          // Length of Moving Average Vector (maximum value for tunable MMGYRO
...
/*************************************************************************************************/
 ...
 /* In Advanced Headfree mode when the copter is farther than ADV_HEADFREE_RANGE meters then 
   ...
   #define ADV_HEADFREE_RANGE 15		//Range in meters where advanced headfree mode activated
  ...
  /************************    Failsafe settings      ************************************/
    ...
    #define RETURN_ALTITUDE    20                     // Return to home altitude in METERS !!
  ...
  /********************************    LED FLASHER    ***********************************/
    ...
    #define LED_FLASHER_SEQUENCE        0b00000000    // while disarmed
    #define LED_FLASHER_SEQUENCE_FIX    0b00000001    // while GPS_FIX OK
    #define LED_FLASHER_SEQUENCE_ARMED  0b11111111    // while armed
  ...
  /**************************************************************************************/
   ...
   /* GPS protocol 
    ...
    /* indicate a valid GPS fix with at least 5 satellites by flashing the LED  - Modified by MIS - Using stable LED (YELLOW on CRIUS AIO) led work as sat number indicator 
      ...
      - Fix and sat no. bellow 5 -> LED On
    ...
    #define MAG_DECLINATION  4.23f               // South Poland (you can set this with ServoMiscConf too)
    ...
    /* GPS navigation settings for FixedWing (planes) */
    // To prevent altitude loss in turns.
    #define ELEVATORCOMPENSATION  80    // Compensate elevator with % of rollAngle. Elevator goes up when plane Roll  
    #define CRUICE_SPEED          30    // desired RTH GROUND speed in km/h (you can set this with ServoMiscConf too)
    // setup minimum throttle level with failsafe rth by setting "Failsafe Throttle" in ServoMiscConf
    #define MAX_AUTO_THROTTLE    (MAXTHROTTLE-100)    // maximum throttle level with auto throttle control (in us)
  ...
  /**************************************************************************************/
    ...
    /*****************************   The type of LCD     **********************************/
      ...
      //#define OLED_DIGOLE     // I2C LCD: OLED DIGOLE
      //#define OSD_DISPLAY       // SERIAL LCD: Display configuration on Misio-OSD
  ...
  /********************************************************************/
    ...
    //if baro+sonar is used for altitude hold
...
/*************************************************************************************************/
  ...
  /***********************             HW PWM Servos             ***********************/ 
    /* HW PWM Servo outputs for Arduino Mega.. moves:
      ...
      // for fixed wings we get additionally:
      SERVO6  = pin 6   (rudder for fixed wing)
      SERVO7  = pin 7   (elevator for fixed wing)
      SERVO8  = pin 8   (motor for fixed wing)       */ 
   
    #define MEGA_HW_PWM_SERVOS
    #define SERVO_RFR_RATE  60    // In Hz, you can set it from 20 to 400Hz, used only in HW PWM mode
  ...
  /********************************************************************/
    ...
    // By disabling this function you can save about 350 bytes of code
    #define ENABLE_MOTORTEST
