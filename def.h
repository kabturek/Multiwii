...
/**************************************************************************************/
#define SERVO_RATES      {30,30,0,100,100,100,100,100}

#if defined (AIRPLANE) || defined(FLYING_WING) || defined(VTAIL_AIRPLANE)
  #define FIXEDWING
#endif
...
#if defined(FIXEDWING) || defined(BI) || defined(TRI) || defined(SERVO_TILT) || defined(GIMBAL) || defined(CAMTRIG) || defined(HELICOPTER) || defined(SERVO_MIX_TILT)|| defined(SINGLECOPTER)|| defined(DUALCOPTER)
  #define SERVO
#endif
...
#elif defined(AIRPLANE) || defined(VTAIL_AIRPLANE)
  #if defined (USE_THROTTLESERVO)
    #define NUMBER_MOTOR   0
  ...
  #define PRI_SERVO_FROM   4
    ...
    #else  
      #define NUMBER_MOTOR   1 // use motor1 for throttle, and optionally motor2 for YAWMOTOR
    #endif  
...
//please submit any correction to this list.
...
#if defined(FFIMUv2) || defined(HK_MultiWii_PRO)
  #define ITG3200
  #define BMA180
  #define BMP085
  #define HMC5883
 ...
 #define FREEIMUv043
...
#if defined(FREEIMUv043) || defined(MICROWII) || defined(MultiWii_PRO_Ez3)
  #define MPU6050
  #define HMC5883
  #define MS561101BA
  #define ACC_ORIENTATION(X, Y, Z)  {imu.accADC[ROLL]  = -X; imu.accADC[PITCH]  = -Y; imu.accADC[YAW]  =  Z;}
  ...
  #define BUZZERPIN_PINMODE          pinMode (A5, OUTPUT);
  #define BUZZERPIN_ON               PORTF |= 1<<5;
  #define BUZZERPIN_OFF              PORTF &= ~(1<<5);
  #define LED_FLASHER_DDR            DDRF    // A6
  #define LED_FLASHER_PORT           PORTF
  #define LED_FLASHER_BIT            PORTF6
#endif
...
#if defined(KUKI_063)    // Experimental, not tested, sensors OK, probably OK, other should be OK.
  #define ITG3200
  #define BMA180
  #define HMC5883
  #define MS561101BA
  ...
  #undef  LED_FLASHER
  #define STABLEPIN_PINMODE          pinMode(12,OUTPUT);  // Yellow D12 LED
  #define STABLEPIN_ON               PORTB |= 1<<4;
  #define STABLEPIN_OFF              PORTB &= ~(1<<4);
  #if defined(FLYING_WING)
    #define SERVO_4_PINMODE          pinMode(3,OUTPUT);  // Flying Wing: Left aileron - M4, Right aileron M3, Motor M1. Define USE_THROTTLE_SERVO for fixedwing !
    #define SERVO_4_PIN_HIGH         PORTD|= 1<<3;
    #define SERVO_4_PIN_LOW          PORTD &= ~(1<<3);
  #else  
    #define SERVO_4_PINMODE            ;    // Standard plane: Ailerons (only one output) M3, Elevator M4, Rudder M2, Motor M1. Define USE_THROTTLE_SERVO for fixedwing !
    #define SERVO_4_PIN_HIGH           ;
    #define SERVO_4_PIN_LOW            ;
  #endif
#endif
  ...
  #define SONAR_GEP_EchoPin_PCMSK          PCMSK0 = (1<<SONAR_GEP_EchoPin_PCINT); // Mask Pin PCINT4 - all other PIns PCINT0-7 are not allowed to create interrupts!
#endif

#if defined(PROFLYER_v31)
  ...
  #define SERVO_1_PIN_HIGH           PORTF |= 1<<0;
  #define SERVO_1_PIN_LOW            PORTF &= ~(1<<0);
  #define SERVO_2_PINMODE            pinMode(A1,OUTPUT);  // TILT_ROLL
  #define SERVO_2_PIN_HIGH           PORTF |= 1<<1;
  #define SERVO_2_PIN_LOW            PORTF &= ~(1<<1);
  #define SERVO_3_PINMODE            pinMode(A2,OUTPUT);  // Cam Trig on PS connector
  #define SERVO_3_PIN_HIGH           PORTF |= 1<<2;
  #define SERVO_3_PIN_LOW            PORTF &= ~(1<<2);
  #undef  LED_FLASHER
  #define VBATSCALE  138
#endif
...
#if defined(ITG3200) || defined(L3G4200D) || defined(MPU6050) || defined(MPU3050) || defined(LSM330) || defined(WMP)
  #define GYRO 1
#else
  #define GYRO 0
#endif
...
#if defined(SRF02) || defined(SRF08) || defined(SRF10) || defined(SRC235) ||defined(SONAR_GENERIC_ECHOPULSE) 
  #define SONAR 1
#else
  #define SONAR 0
#endif
...
#define ACC_VelScale (9.80665f / 10000.0f / ACC_1G)
 ...
 #define GYRO_SCALE ((4.0f * PI * 70.0f)/(1000.0f * 180.0f * 1000000.0f))
  ...
  #undef THROTTLE_ANGLE_CORRECTION
  #undef ADVANCED_HEADFREE
  #undef GPS_FILTERING
  #undef GPS_LEAD_FILTER
  #undef ACROTRAINER_MODE
#endif
...
#elif defined(BI)
  #define MULTITYPE 4
  #define SERVO_RATES      {30,30,0,100,0,1,100,100}
#elif defined(GIMBAL)
  #define MULTITYPE 5
#elif defined(Y6)
  #define MULTITYPE 6
...
#elif defined(FLYING_WING)
  #define MULTITYPE 8
  #define SERVO_RATES      {30,30,0,0,1,0,0,100}
#elif defined(Y4)
  #define MULTITYPE 9
#elif defined(HEX6X)
  #define MULTITYPE 10
...
#elif defined(SINGLECOPTER)
  #define MULTITYPE 20
  #define SERVO_RATES      {30,30,0,0,1,0,1,100}
#elif defined(DUALCOPTER)
  #define MULTITYPE 20
#elif defined(VTAIL_AIRPLANE)  
  #define MULTITYPE 21    
  #define SERVO_RATES      {30,30,0,100,-100,2,3,100}
#endif
...
#if defined (AIRPLANE) || defined (VTAIL_AIRPLANE) || defined(HELICOPTER) || defined(SINGLECOPTER) || defined(DUALCOPTER) && defined(PROMINI) 
...
#if defined(SBUS)
  #define RC_CHANS 18
#elif defined(SPEKTRUM) || defined(SERIAL_SUM_PPM) || defined(ELERES_RX)
  #define RC_CHANS 12
#else
  #define RC_CHANS 8
#endif
...
///////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  #if (defined(LCD_VT100)) || (defined(OLED_I2C_128x64) || defined(OLED_DIGOLE) || defined(OSD_DISPLAY) )
...
#if (defined(LCD_DUMMY) || defined(LCD_SERIAL3W) || defined(LCD_TEXTSTAR) || defined(LCD_VT100) || defined(LCD_TTY) || defined(LCD_ETPP) || defined(LCD_LCD03) || defined(OLED_I2C_128x64) || defined(OLED_DIGOLE) || defined(OSD_DISPLAY) )
  #define HAS_LCD
#endif
...
//don't use BARO with small 32k devices on airplane modes with serial GPS
#if !defined(MEGA) && defined(FIXEDWING) && GPS
  #define BARO 0
  #undef BMP085
  #undef MS561101BA
#endif  
