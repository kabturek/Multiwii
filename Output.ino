...
/**************************************************************************************/
  ...
  #endif
#else
  #if defined(SERVO) && !(defined(MEGA) && defined(MEGA_HW_PWM_SERVOS))
    #if defined(AIRPLANE) || defined(HELICOPTER)
      // To prevent motor to start at reset. atomicServo[7]=5 or 249 if reversed servo
...
void writeMotors() { // [1000;2000] => [125;250]
  ...
  if(f.ARMED || motorTestNumber > 8) motorTestNumber = 0;
  if(motorTestNumber && motorValueData > MINCOMMAND && motorValueData <= MAXTHROTTLE) { // if motortest enabled, and motor value in propper range
     for(uint8_t i=0; i<8; i++) motor[i] = MINCOMMAND;  // clear all motors values
     if(++motorTestTimeout > 250)                       // check for UART communication timeout
       motorTestNumber = 0;                             // disable motortest if no uart communication
     else 
       motor[motorTestNumber-1] = motorValueData;  // update current selected motor value from MSP_SET_MOTOR data
  }
#endif  
...
void mixTable() {
  ...
  /**************************   AIRPLANE with V-TAIL and Ailerons   *******************************/
  // supported flaps and flaperons too
    ...
    if (f.PASSTHRU_MODE) {         // do not use sensors for correction, simple 2 channel mixing
      servo[3] = rcCommand[ROLL] + flapperons[0];                                           // Wing 1
      servo[4] = rcCommand[ROLL] + flapperons[1];                                           // Wing 2
      servo[5] = (SERVODIR(5,1) * rcCommand[PITCH]) + (SERVODIR(5,2) * rcCommand[YAW]);     // Elevator/rudder MIX 
      servo[6] = (SERVODIR(6,1) * rcCommand[PITCH]) + (SERVODIR(6,2) * rcCommand[YAW]);     // Elevator/rudder MIX
    } else {                       // use sensors to correct (gyro only or gyro+acc according to aux1/aux2 configuration
      servo[3] = axisPID[ROLL] + flapperons[0];                                             //   Wing 1
      servo[4] = axisPID[ROLL] + flapperons[1];                                             //   Wing 2
      servo[5] = (SERVODIR(5,1) * axisPID[PITCH])   + (SERVODIR(5,2) * axisPID[YAW]);       // Elevator/rudder MIX
      servo[6] = (SERVODIR(6,1) * axisPID[PITCH])   + (SERVODIR(6,2) * axisPID[YAW]);       // Elevator/rudder MIX
    }
    servo[3]  = ((int32_t)conf.servoConf[3].rate * servo[3])/100L;  // Ailerons servo rates
    servo[4]  = ((int32_t)conf.servoConf[4].rate * servo[4])/100L;  // Ailerons servo rates
    for(i=3;i<7;i++) servo[i] += get_middle(i);
...
/****************                    Cam trigger Sevo                ******************/
    ...
    if(SERVODIR(2,1) == 1) {
      ...
      servo[2] = (camState==1) ? conf.servoConf[2].min : conf.servoConf[2].max;
    }
  #endif
